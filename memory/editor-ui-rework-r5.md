...
**検証状況**: ユーザーが実機で確認済み（2026-08-02）。修正後、ホバー切替時に1つだけ表示されることを確認。

## 実機確認完了（2026-08-02）

§1〜§9の全項目（バンド高さ・レーン中央揃え・設定の永続化・設定モーダル3タブ・
コマンドテーブル化とショートカット・自前メニューとホバー切替、および上記のホバー切替バグ修正）を
ユーザーが実機で確認し、**問題なし**と確認済み。コミット`5d249cf`。

**残件（r4から持ち越し）**: r4 §12（音源オフセット`@OFFSET`の符号の向き）は音源ファイルを
まだ用意していないため検証延期のまま。
