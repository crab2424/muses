...
### 決着（2026-08-13）: 仮説①で確定。streamAudio化は無罪

実機計測の結果は **曲が流れていない間は 0ms、流れている間は -20ms 前後**。

**-20ms は DSPバッファ1個分(1024/44100 = 23.2ms)とほぼ一致する。** これは計測手法上
必ず出る読み取り誤差であって、実際の再生開始遅れではない:

- `musicSource.time` は **DSPブロック境界でしか更新されない**（＝23.2ms刻みの階段）。
- 一方 `smoothed`(songTime) は `Advance()` が毎フレーム補間して**連続的に進む**。
- よって両者の差は常に「最後のブロック境界からの経過分」だけ負に振れ、
  平均すると**約 -12〜-23ms に落ち着く**。観測値 -20ms はこの範囲そのもの。

**仮説②なら -100ms 前後が出るはずだった。出ていない。**
つまり音源は予約どおりの位置で鳴っており、**`streamAudio` 化は音ズレの原因ではない**
（メモリ -40MB はそのまま維持してよい。§3の懸念は杞憂だった）。

したがって体感の約100msは**仮説①のオーディオ出力レイテンシ**——
`AudioSettings.dspTime` が「今ミキシング中の位置」であって「今耳に届いている位置」では
ないことによる、**streamAudio化以前から存在していた性質**である。
音と判定は同じdspTime軸に載っているので内部的には整合しているが、
**画面の描画だけはこのレイテンシを経由しない**ため、視覚が聴覚に先行する。

### 対処: オフセット校正（機能は既にある。符号を以下で確定した）

**song-play-flow-r1.md §10.2 の積み残し「`visualOffsetMs`の符号未検証」もここで解消する。**
コードから追った結果は以下のとおり（L = 出力レイテンシ）。

**描画オフセット**: `VisualTime() = songTime + visualOffsetMs`。
`NoteView.UpdateScroll(VisualTime())` → `_GroupX` → シェーダの
`d0 = _ZJudge + (noteX - groupX) * _Speed`。ノーツが判定線に届く(`d0 == _ZJudge`)のは
`groupX == noteX` すなわち `VisualTime() == noteTime` のとき。よって

```
ノーツが判定線に到達する songTime = noteTime - visualOffset
```

**負の値ほどノーツの到達が遅くなる**。耳に届くのは `noteTime + L` なので、
**`visualOffsetMs = -L`（負）** で音と映像が一致する。

**判定オフセット**: `JudgeTime() = songTime + judgeOffsetMs`、
`Judge.ApplyJudgement` の `ms = 入力時刻 - ノーツ時刻`（**正 = 遅押し / 負 = 早押し**）。
音に合わせて叩くと入力は `songTime = noteTime + L` に来るので
`ms = L + judgeOffset`。これを0にするには **`judgeOffsetMs = -L`（負）**。

**両方とも -L（負）で一致する。** 視覚を音に合わせた上で叩けば判定も正しくなる、
という整合した状態になる。

> **符号のコメント誤りを1件修正した（2026-08-13、コミット同梱）**:
> `Judge.cs:365` の `float ms = -dt * 1000f; // 正 = 早押し` は**逆**だった
> （`dt = noteTime - inputTime` なので `ms = inputTime - noteTime` ＝ 正は遅押し）。
> **この値の符号を見てオフセットを校正する**ため、実害のある誤りだった。

### L の実測手順（HUDだけでできる。追加実装不要）

1. 判定オフセットを **0** にする（設定 → 判定オフセット）。
2. **音だけを頼りに**（画面のノーツを見ずに）拍に合わせて叩く。
3. HUDの `PERFECT +XXms` / `GOOD +XXms` の値を数回読んで平均する。
   これが **そのまま L** になる（`ms = L + judgeOffset`、judgeOffset=0 のため）。
   出力レイテンシがあるなら**必ず正（遅押し側）に偏る**はず。
4. **判定オフセット・描画オフセットの両方に `-L`（負の値）** を入れる。
5. 今度は画面を見て叩き、`ms` が0付近に散らばることを確認する。

### L 自体を減らしたい場合（トレードオフ、未実施）

校正で吸収するのが基本だが、L そのものを下げる手もある:

- **`AudioManager.m_DSPBufferSize` を 1024 → 512 か 256 に下げる**。
  1024は「Best performance」で、1段あたり23.2ms。512なら11.6ms、256なら5.8ms。
  **副次的に dspTime の更新レートも上がる**（23〜25Hz → 倍・4倍）ので時計も素直になる。
  **代償はCPU負荷**。§11でCPUが主役になった直後なので、ここを重くするのは方向が逆。
  §5【E】§6【F】でCPUを削ってから触るのが順序として正しい。
- **Bluetooth接続で聴いている場合は打つ手が無い**。BTは規格上100〜200msの
  レイテンシがあり、DSPバッファをいくら下げても消えない。**校正一択**。
  内蔵スピーカ/有線なら L は50〜80ms程度に収まるのが普通で、
  体感100msという報告はBluetoothを疑う理由になる。
...
2. ~~**C（`streamAudio`）**~~ → **実装済み。-40MBを確認**（§11）。
   ただし**§12のズレとの関係が未確定**。
3. ~~**§12 のズレの切り分け**~~ → **2026-08-13決着（§12）。実測 -20ms ≒ DSPブロック1個分で、
   streamAudio化は無罪**。正体は従来からあるオーディオ出力レイテンシ。
   **メモリ-40MBはそのまま維持してよい。**
   → 残作業は**ユーザーによる L の実測とオフセット校正**（§12「Lの実測手順」）。
4. 主役が CPU に移ったので、次は **E（オーバーレイの dirty ガード）→ F（OnGUI廃止）**。
   **オフセット校正はユーザーが完了（2026-09-25）。E は §5 対処1を実装済み（2026-09-25、実機での効果測定はまだ）**:
