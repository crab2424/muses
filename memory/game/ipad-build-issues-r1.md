...
### ②-B 追記（2026-08-06、実機確認後）: v1実装のクランプが新たな周期的スタッタリングを生んだ → v2で解消

v1実装（`SongClock.Advance()`にクランプを入れ、`smoothed`が`authoritative + SnapThreshold`(50ms)を
超えたら値を据え置く方式）を実機確認したところ、**処理落ちによる固まりではなく、120fps基準で
3〜4フレーム分・秒間数回、周期的にフレームが飛ぶ**という新しい症状が出た。

**原因**: DSP更新間隔（実測約23〜25Hz＝約40〜43ms）とクランプの許容幅（50ms）の差が
わずか7〜10msしかなかった。ビデオクロック（`Time.unscaledDeltaTime`の積算）とオーディオクロック
（`dspTime`）は本質的に別のクロック源で、わずかなジッタ・クロック速度差があるのが普通。
この薄い7〜10msの余白はDSPコールバックのタイミングジッタで簡単に食いつぶされ、
`smoothed`がクランプ値に張り付いて**数フレーム凍結 → dspTime更新で一気に追いつく**、
という周期的な「フレーム飛び」を引き起こしていた。

**修正（v2）**: クランプ（値を据え置く経路）を完全に廃止。`smoothed`は**毎フレーム必ず
deltaTime分だけ加算**し、dspTimeが更新されたフレームでは**追加でズレを加算/減算**するだけに
変更（大きくズレていれば全部、小さければ`DriftCorrectionRate`ぶんだけ）。
すべて加算なので「値が据え置かれる経路」自体が存在しなくなり、周期的な凍結は原理的に起きない。

**How to apply**: フレーム補間・ドリフト補正コードで「一定範囲を超えたら値を据え置く」
クランプを入れるときは、クランプの許容幅と実際の更新間隔（ジッタ込み）の余裕を十分に取ること。
余裕が薄いと、正常なジッタだけで周期的にクランプが作動し、かえって新しいスタッタリングを生む
（今回は50ms許容 vs 40〜43ms間隔で7〜10msしか余裕が無かったのが原因）。
「据え置く」形のクランプそのものを避け、「常に前進・ズレは加算で補正」の形にすると
この種のバグを構造的に避けられる。
