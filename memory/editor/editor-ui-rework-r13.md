...
> **残る不整合（ユーザー判断待ち）**: `SampleScene.unity`（ゲーム本体）の `thicknessMinFrac` は
> **0.004 のまま**。プレビューは 0.01 になったので、§7.1 で問題にした「コード既定とシーンの
> 二重管理によるドリフト」が別の値で再発している状態。ゲーム側の見た目を変える判断になるため
> 勝手には合わせない。§10-2 の ScriptableObject 化で根本解決するのが本筋。
