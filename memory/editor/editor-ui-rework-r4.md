...
## §7 続報: 実機確認の結果、色の修正だけでは直らず（2026-08-02、コミット`c5f484b`の直後）

`e97e70a`直後の色修正（`--text`色の明示）を実機確認したが**文字はまだ見えなかった**。
ユーザーが「文字は描画されているが余白で隠れているのでは」という仮説を提示し、根拠として
インスペクタの `layerF` 行のスクリーンショットで**値欄の一部が見切れて(隠れて)いる**ことを指摘した。
`入力枠が被っていて崩れている箇所もある`（次回対応の別件）という報告とあわせて、
**単なる色の問題ではなくレイアウトの重なりが本質**と判断し、原因を再調査した。

### 特定した原因（2つ、両方修正）

1. **`.prop-row { height: 20px }` が固定値だったため、ランタイム既定テーマの
   `TextField`系コントロールの実際に必要な高さ（内部padding込み）に足りず、はみ出した分が
   **次の行の不透明な背景（`--bg-control`）に隠れて**いた。** これがユーザーの言う
   「入力枠が被っている」崩れと同一原因であり、「文字が見えない」の主因でもあった
   （テキスト自体は描画されているが、後で描かれる次の行のボックスに覆われて見えなくなる）。
   `height: 20px` を `min-height: 20px` に変更し、行の高さがコントロールの実測サイズに
   追従するようにして解消した。
2. **CSSの継承の優先順位の見落とし**: `color` は継承プロパティなので、先の修正では
   `.unity-base-field__input`（入力欄のコンテナ）に `color` を当てていた。しかし
   Unity既定のランタイムテーマが内部の実テキスト要素（`.unity-text-element`、UIElementsModule.dll
   の文字列テーブルで存在を確認済み）へ**直接** `color` を明示している場合、
   **継承値はどんなに詳細度の高いルールで祖先に当てても、子孫への明示指定には必ず負ける**
   （継承はCSSカスケードの中で最も優先度が低い）。実テキスト要素へ直接
   `.prop-row .unity-base-field__input .unity-text-element { color: var(--text); }` を
   当てて確実に上書きするよう変更した。

**How to apply**: 今後UI Toolkitでランタイム既定テーマの上から色を上書きするときは、
祖先要素に`color`を置くだけでなく、実際にテキストを保持する`.unity-text-element`まで
届いているか（テーマが子孫に直接colorを明示していないか）を疑う。また固定`height`を使う行に
標準コントロールを置くときは、テーマ側の実測サイズを保証できないので`min-height`を既定にする。

**検証状況**: ユーザーが実機で確認済み。文字・カーソルとも正しく見えるようになった。

## 実機確認完了（2026-08-02）

§1〜§11（§12除く）を含む全項目をユーザーが実機で確認し、**問題なし**と確認済み。
§12（音源オフセット、`@OFFSET`の符号の向き）だけは**音源ファイルを何も用意していないため
検証を延期**（用意でき次第、別セッションで確認）。

**次にやること**: 続きは別セッションで着手。r4で扱わなかった残件は
[[muses-unity-port-progress]] を参照（設定画面・容量調査・波形表示など）。
